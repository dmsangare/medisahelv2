    ...
    <div className="space-y-6" id="agenda-view-wrapper">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4">
          ...
          <div className="bg-slate-50 p-3.5 border border-slate-200 rounded-lg text-[10.5px] text-slate-500 space-y-2 leading-relaxed">
            <span className="font-bold text-slate-700 block uppercase tracking-wider flex items-center gap-1">
              <MessageSquare className="h-3 w-3 text-emerald-600" /> Relances Client
            </span>
            <p>La passerelle envoie de façon synchrone en local :</p>
            ...
            {filteredAppts.length === 0 ? (
              ...
              <div className="space-y-3">
                {filteredAppts.map((appt) => {
                  ...
                  return (
                    <div key={appt.id} className="p-4 bg-slate-50/50 border border-slate-200 rounded-xl relative hover:bg-slate-50 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs font-medium">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className={`px-2.5 py-0.5 rounded-full border text-[9.5px] font-extrabold ${stateStyles}`}>
                            {appt.statut}
                          </span>
                          <span className="font-bold text-slate-900 text-sm">{appt.patientNom}</span>
                          <span className="text-[10px] text-slate-400 font-mono">({appt.patientId})</span>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 pt-1 text-[11px] text-slate-500 font-mono">
                          <span className="flex items-center gap-1.5 font-sans"><User className="h-3.5 w-3.5 text-slate-400" /> Praticien : <strong>{appt.medecin}</strong></span>
                          <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5 text-slate-400" /> Heure : <strong>{appt.heure}</strong></span>
                          <span className="flex items-center gap-1.5"><Home className="h-3.5 w-3.5 text-slate-400" /> Salle allocation : <strong>{appt.salle}</strong></span>
                        </div>

                        {appt.notes && (
                          <p className="text-[11px] italic text-slate-400 pt-1">Motif de consultation: {appt.notes}</p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 self-end md:self-center">
                        <button
                          ...
                          className="p-1 px-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded border border-emerald-200 text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-all"
                        >
                          <CheckCircle2 className="h-3 w-3" /> Terminer
                        </button>
                        <button
                          onClick={() => onUpdateStatus(appt.id, "Reporté")}
                          className="p-1 px-2.5 bg-amber-50 hover:bg-amber-100 text-amber-800 rounded border border-amber-250 text-[10px] font-bold cursor-pointer transition-all"
                        >
                          Reporter
                        </button>
                      </div>
